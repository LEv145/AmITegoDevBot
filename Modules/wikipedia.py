...
class BotWikipedia(commands.Cog):
    ...
    @commands.command(
        ...
        usage="избранное <None>"
    )
    async def favorites(self, ctx):
        wikipedia.set_lang('ru')
        text = wikipedia.summary('Шаблон:Текущая избранная статья')
        e = discord.Embed(description=text, colour=discord.Color.light_grey())
        e.set_author(name=f'Избранная статья',
                     icon_url='https://media.discordapp.net/attachments/640123959135698954/711188654806007818/seo-and-web.png?width=407&height=407')
        e.set_footer(text='Wikipedia',
                     icon_url='https://upload.wikimedia.org/wikipedia/en/thumb/8/80/Wikipedia-logo-v2.svg/1200px-Wikipedia-logo-v2.svg.png')
        await ctx.send(embed=e)
